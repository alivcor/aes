...
def performLSA(essay_fn, data_fn):
    count = 0
    esslist = []
    # ess3txt = ""
    # f = open('Dataset/Set1Complete.csv', 'rb')
    perfectEssayFileReader = open('Dataset/'+essay_fn, 'rb')
    resslist = []
    esstxts = []
    term_freq_matrix = []
    # esstxts.append(
    #     "@ORGANIZATION1, Computers are great tools and a great piece of modern technology. Almost every family has them. About @PERCENT1 of my class has computers. So many people have them because their helpful and another current learning resource. Also it's a gr")
    # esstxts.append(
    #     "I go out on sunday nights to play football. We all like sports. Hockey is also a sport. Badminton is also one of my favorite sports")
    # esstxts.append(
    #     "Do you love to cook? Cooking Channel's shows and top global chefs share their best recipes and demonstrate their specialties in cooking technique videos.")
    # esstxts.append(
    #     "I like computers. Computers provide a lot of help to the users through effective use of technology. Email and messaging and social media are enabled only because of computers")
